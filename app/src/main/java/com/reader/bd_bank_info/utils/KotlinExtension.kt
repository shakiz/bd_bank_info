...
fun getLocalizedText(isBangla: Boolean, textEn: String? = null, textBn: String? = null): String {
    if (isBangla && textBn.isNullOrBlank().not()) {
        return textBn.orEmpty()
    }

    return textEn.orEmpty()
}
...
fun Context?.isBanglaLocale(): Boolean {
    this ?: return false
    return currentLanguage() == BANGLA
}
...
fun NavController.find(deeplink: Uri): NavDestination? {
    graph.iterator().forEach {
        if (it.hasDeepLink(deeplink)) {
            return it
        }
    }
    return null
